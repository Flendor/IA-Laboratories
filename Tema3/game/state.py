...
class State:
    ...
    def __init__(self,
        ...
        if human_pieces is None:
            _human_pieces = [
                game.Piece(x=x, y=y, directions=game.Directions.HUMAN, player_type=game.PlayerTypes.HUMAN, state=self)
                for
                x, y in self.__initial_human_pieces_positions]
        else:
            _human_pieces = [game.Piece(x=human_piece.x, y=human_piece.y, directions=game.Directions.HUMAN,
        ...
        if bot_pieces is None:
            _bot_pieces = [
                game.Piece(x=x, y=y, directions=game.Directions.BOT, player_type=game.PlayerTypes.BOT, state=self)
                for
                x, y in self.__initial_bot_pieces_positions]
        else:
            _bot_pieces = [game.Piece(x=bot_piece.x, y=bot_piece.y, directions=game.Directions.BOT,
